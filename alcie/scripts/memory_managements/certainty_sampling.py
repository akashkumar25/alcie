...
# ===============================
class MemoryBufferCertainty:
    def __init__(self, capacity=550, sampling_strategy="certainty"):
        self.buffer = {}
    ...
    def _get_lowest_score(self, cluster_id):
        ...
        lowest_score_sample = min(cluster_samples, key=lambda x: x['certainty_score'])
        return lowest_score_sample['certainty_score']
        
    def push(self, sample, cluster_id, score):
        sample['cluster_id'] = cluster_id
        sample['certainty_score'] = score
    ...
    def _add_sample(self, image_id, sample, score, cluster_id):
        ...
        print(f"Added sample with certainty score {score} from cluster {cluster_id}. Current buffer size: {len(self.buffer)} samples.\n")
        with open(self.log_file, 'a') as f:
            f.write(f"Added sample with certainty score {score} from cluster {cluster_id}. Current buffer size: {len(self.buffer)} samples.\n")
            
    def free_space(self, cluster_id, num_to_delete=1):
        """
        Free a fraction of the memory by deleting samples with the lowest certainty scores
        ...
        # Sort the samples in descending order (lowest certainty score first).
        sorted_samples = sorted(cluster_items, key=lambda x: x[1]['certainty_score'])
        ...
        print(f"Deleted {num_to_delete} sample(s) with the lowest certainty scores from cluster {cluster_id}.")
        with open(self.log_file, 'a') as f:
            f.write(f"Deleted {num_to_delete} sample(s) based on lowest certainty scores from cluster {cluster_id}.\n")
    ...
    def _delete_from_cluster(self, cluster, delete_percent):
        """
        Delete a percentage of the samples from the specified cluster, based on the lowest certainty scores.
        ...
        # Sort samples by certainty score in ascending order
        sorted_entries = sorted(cluster_entries, key=lambda key: self.buffer[key]['certainty_score'])
...
# ===============================
# certainty Memory Replay Trainer
# ===============================
class CertaintyMemoryReplayTrainer(Trainer):
    ...
    def training_step(self, model, inputs):
        inputs_for_model = {k: v for k, v in inputs.items() if k not in ['caption','image_ids']}
        loss = super().training_step(model, inputs_for_model)

        self.total_samples_seen += len(inputs_for_model['input_ids'])

        if self.memory_buffer is not None:
            self.select_certain_samples_with_clip(inputs)
        ...
        if self.use_memory_replay and len(self.memory_buffer) > 0:
            if self.total_samples_seen >= self.replay_freq:
                ...
                clean_memory_batch = {k: v for k, v in memory_batch.items() if k not in ['cluster_id', 'certainty_score','image_ids']}
    ...
    def select_certain_samples_with_clip(self, batch, k=64):
        patch_images = batch['patch_images']
        captions = batch['caption']

        # Move the image tensors to the target device.
        patch_images_device = patch_images.to(self.device)
        # Tokenize and move the captions to the target device.
        ...
        # Get indices of the top k samples (or fewer if the batch is smaller).
        best_indices = similarities.topk(k=min(len(similarities), k)).indices.tolist()

        for best_index in best_indices:
            # Define the score for this sample as its similarity value.
            score = similarities[best_index].item()
            sample = {
                'patch_images': patch_images[best_index].unsqueeze(0).cpu().half(),
    ...
    def prediction_step(self, model, inputs, prediction_loss_only, ignore_keys=None):
        inputs = {k: v for k, v in inputs.items() if k not in ['caption','image_ids']}
