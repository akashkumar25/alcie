...
class MemoryBufferRandom:
    ...
    def push(self, sample, cluster_id):
        ...
        if image_id in self.buffer:
            ...
            if len(self.buffer) >= self.capacity:
                ...
                self._add_sample(image_id, sample, cluster_id)
               
    def _add_sample(self, image_id, sample, cluster_id):
        self.buffer[image_id] = sample
        print(f"Added sample randomly from cluster {cluster_id}. Current buffer size: {len(self.buffer)} samples.\n")
        with open(self.log_file, 'a') as f:
            f.write(f"Added sample randomly from cluster {cluster_id}. Current buffer size: {len(self.buffer)} samples.\n")
    ...
    def _delete_from_cluster(self, cluster, delete_percent):
        ...
        random_samples = random.sample(cluster_entries, num_to_delete)
...
class RandomMemoryReplayTrainer(Trainer):
    def __init__(self, *args, memory_buffer=None, replay_freq=200, use_memory_replay=False,
        ...
        super().__init__(*args, **kwargs)  # Ensure Hugging Face Trainer is initialized
        self.memory_buffer = memory_buffer
        self.replay_freq = replay_freq
        self.device = device
        self.use_memory_replay = use_memory_replay
        self.current_cluster = current_cluster
        self.total_samples_seen = 0  # Counter to track processed samples


    def training_step(self, model, inputs, num_items_in_batch=None):
        # Remove 'caption' from inputs and perform standard training
        inputs_for_model = {k: v for k, v in inputs.items() if k not in ['captions','image_ids']}
        loss = super().training_step(model, inputs_for_model, num_items_in_batch=None)

        # Update total samples seen
        self.total_samples_seen += len(inputs_for_model['input_ids'])
        
        # Apply memory storage logic with 20% chance per item
        if self.memory_buffer is not None:
            self.random_sampling_with_chance(inputs)  # Store up to 16 items with 20% chance

        # Perform memory replay if enabled
        if self.use_memory_replay and len(self.memory_buffer) > 0:
            if self.total_samples_seen >= self.replay_freq:
                self.total_samples_seen = 0

                memory_samples = self.memory_buffer.sample_by_cluster(1, exclude_cluster=self.current_cluster)
                if memory_samples:
                    memory_batch = self.collate_samples(memory_samples)
                else:
                    print('Not able to sample data from previous clusters')
                clean_memory_batch = {k: v for k, v in memory_batch.items() if k not in ['cluster_id','image_ids']}
                ...
                loss += memory_loss
                print("Completed memory replay step.")

        return loss

    def random_sampling_with_chance(self, batch):
        # Store items with 20% chance, up to 16 items maximum
        selected_indices = [
            i for i in range(len(batch['pixel_values']))
            if random.randint(0, 9) < 2  # 20% chance for each item
        ]
        ...
        if not selected_indices:
            print("No items selected for memory storage from this batch.")
            return
        
        for indices in selected_indices:
            # Create a clean batch with selected items
            sample = {
                # 'patch_images': batch['patch_images'][indices].unsqueeze(0).cpu().half(),
                'pixel_values': batch['pixel_values'][indices].unsqueeze(0).cpu().half(),
                'input_ids': batch['input_ids'][indices].unsqueeze(0).cpu(),
                # 'decoder_input_ids':batch['decoder_input_ids'][indices].unsqueeze(0).cpu(),
                'attention_mask': batch['attention_mask'][indices].unsqueeze(0).cpu(),
                'image_ids': batch['image_ids'][indices],
                'labels': batch['labels'][indices].unsqueeze(0).cpu()
                # 'return_loss': True
            }

            # Push the clean batch to the memory buffer with the current cluster ID
