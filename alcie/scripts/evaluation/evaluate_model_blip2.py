...
def preprocess_image(image_content):
    ...
    return image  # Return the raw PIL image without transformations
...
# ========================
def generate_caption(image_contents, processor, model):
    batch_images = [preprocess_image(content) for content in image_contents]  # No transformations
    inputs = processor(images=batch_images, return_tensors="pt").to(device)  # Processor does preprocessing
    pixel_values = inputs.pixel_values
    gen = model.generate(pixel_values, num_beams=5, no_repeat_ngram_size=3)
...
def process_metrics(data):
    bleu4 = data["BLEU"].get("precisions", [None])[-1]
    rougeL = data["ROUGE"].get("rougeL", None)
    meteor = data["METEOR"].get("meteor", None)
    bert_precision_avg = sum(data["BERTScore"].get("precision", [0])) / len(data["BERTScore"].get("precision", [1]))
    bert_recall_avg = sum(data["BERTScore"].get("recall", [0])) / len(data["BERTScore"].get("recall", [1]))
    bert_f1_avg = sum(data["BERTScore"].get("f1", [0])) / len(data["BERTScore"].get("f1", [1]))
    return {
        "BLEU4": bleu4,
        "ROUGE_L": rougeL,
        "METEOR": meteor,
...
def main():
    ...
    base_model_path = f"/home/akkumar/ALCIE/fine_tuned_models/blip2/temp/{sampling_method}/BLIP2_trained_model_memory"
    
    print("Evaluating Model:", base_model_path)
    
    test_data_path = "/home/akkumar/ALCIE/cluster_facade_training_combined"
    eval_results_path = f"/home/akkumar/ALCIE/evaluation/blip2/temp/{sampling_method}"
    pred_results_path = f"/home/akkumar/ALCIE/prediction/blip2/temp/{sampling_method}"
